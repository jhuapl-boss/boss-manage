...
@contextmanager
def create_tunnels(targets, local_port = None):
    """
    NOTE: The last target in targets is not actually connected to, its information is
          used and yielded for the final connection

    NOTE: local_port is only used if two or more targets are given. If only one target
          is given then no SSH tunnels need to be created, as the caller can connect
          directly

    Args:
        targets (list[SSHTarget]): List of target machines to create tunnels to
        local_port (int): Local port for the final connection

    Yields:
        SSHTarget: The target information for the final SSH connection
    """
    print("create_tunnels({}, {})".format(targets, local_port))
    if len(targets) == 0:
        raise ValueError("No targets defined")

    if len(targets) == 1:
        # No SSH Tunnels needed to connect to the target machine
        if __debug__:
            target = targets[0]
            print("Connect to {}@{}:{}".format(target.user, target.ip, target.port))
        print("No bastions defined, connecting directly")
        yield targets[0]
        return

    ########################################################################
    # DP NOTE: Whats happening here is that bastion N is used to form a    #
    #          tunnel to bastion N+1. This process happens until the       #
    #          target machine is reached. When linking each of the tunnels #
    #          together the local port of the previous tunnel is used when #
    #          making the next connection                                  #
    ########################################################################

    local_ports = []
    bastions = [] # (key, user, ip, port)
    remotes = [] # (ip, port)
    ...
    # Add the initial bastion machine's information
    bastions.append((targets[0].key,
                     targets[0].user,
                     targets[0].ip,
                     targets[0].port))

    # Chain the following bastions together using different local ports
    for target in targets[1:-1]:
        port = locate_port()
        local_ports.append(port)
        bastions.append((target.key, target.user, "localhost", port))
        remotes.append((target.ip, target.port))
        # localhost:port is now an alias for target.ip:target.port

    # Finally add the target information for the final SSH tunnel
    # This just defines a tunnel to the target and doesn't connect
    # to the machine itself, that is done by the caller
    local_ports.append(local_port or locate_port())
    remotes.append((targets[-1].ip, targets[-1].port))

    # Information for the caller to use when forming the final connection
    # through the established tunnels
    target = SSHTarget(targets[-1].key, "localhost", local_ports[-1], targets[-1].user)

    # Create each SSH tunnel and add it to the list of open processes
    # to close when the tunnel is finished
    wrapper = ProcWrapper()
    for i in range(len(bastions)):
        l_p = local_ports[i]
        b_k, b_u, b_i, b_p = bastions[i]
        r_i, r_p = remotes[i]

        # Some number of bastions may already have tunnels setup, skip setting them up
        key = (r_i, r_p)
        if key in context:
            context = context[key]
            continue

        # Save the context where we start creating tunnels so we can remove it when the tunnels are closed
        if _context is None:
            _context = context
            _key = key
            if __debug__:
                print("Starting with key {}".format(key))
            if i > 0:
                b_p = context.local_port

        if __debug__:
            print("Connecting {} -> ({}@{}:{}) -> {}:{}".format(l_p, b_u, b_i, b_p, r_i, r_p))

        try:
            context[key] = l_p # Needed here for error handling
            context = context[key]

            proc = create_tunnel(b_k, # bastion key
                                 l_p, # local port
                                 r_i, # remote ip
                                 r_p, # remote port
                                 b_i, # bastion ip
            ...
            # remove the connections from the context
            del _context[_key]
    ...
    try:
        if _key is None:
            target.port = context.local_port
...
class ContextNode(dict):
    def __init__(self, parent, local_port):
        self.parent = parent
        self.local_port = local_port

    def __setitem__(self, key, value):
        if not isinstance(value, ContextNode):
            value = ContextNode(self, value)
        return super().__setitem__(key, value)

    def __str__(self, indent=0):
        children = ["{}{}:{}".format(' '*(indent+2), key, self[key].__str__(indent+2))
                    for key in self]
        return "{} -> {{\n{}\n{}}}".format(self.local_port, "\n".join(children), ' '*indent)
CONTEXT = ContextNode(None, 0)

class SSHTarget(object):
    """Object for containing information about a machine to connect to via SSH

    Args:
        key (string): Path to SSH private key used for authentication
        ip (string): IP address of the machine
        port (string|int): Port on the machine
        user (string): Username of the user that matches the private key
    """
...
class SSHConnection(object):
    """Object for connecting to a remote machine via SSH using public keys

    NOTE: The list of bastions has not required or fixed size, though each
          bastion machine connection adds a 10 second sleep to verify that
          the SSH tunnel was created and established.

    Args:
        target (SSHTarget): Target machine to connect to
        bastions (SSHTarget|list[SSHTarget]): List of machines though which to
                                              create SSH tunnels so that the
                                              target machine can be reached
        local_port (int): Local port number to use for SSH tunnels
                          If not provided, one will be randomly picked
    """
    ...
    def _connect(self):
        ...
        If no bastion machines were defined in the constructor a message is
        printed and the target machine information is yielded so dependent
        methods can connect directly to the target machine.

        Yields:
            SSHTarget: Object containing the information needed to connect to
                       the target machine, either directly or via SSH tunnel(s)
        """
        wrapper = ProcWrapper()
        ...
        with create_tunnels(self.bastions + [self.target], self.local_port) as target:
            yield target
        return
        # DP NOTE: Original code kept until new code is tested
